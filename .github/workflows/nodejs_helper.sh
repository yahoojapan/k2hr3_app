...
#
# Check npm help and build command
#
echo "[INFO] ${PRGNAME} : Check npm help and build command."
...
if [ $? -eq 0 ]; then
	...
	# Start build
	#
	echo "[INFO] ${PRGNAME} : Start build."
	run_cmd npm run build
else
	echo "[INFO] ${PRGNAME} : There is no build command, so skip build."
fi
...
if [ ${DO_NOT_RUN_TEST} -eq 1 ]; then
	...
	if [ ${IS_BUILD_DEMO} -eq 1 ]; then
		export PUBLISH_TAG_NAME=${PUBLISH_TAG_NAME}
		export GHPAGES_WITHOUT_LICENSE=1
