...
#
# This script is image builder for NodeJS project without binary building
#
#---------------------------------------------------------------------
func_usage()
{
...
while [ $# -ne 0 ]; do
	...
	elif [ "X$1" = "X-g" ] || [ "X$1" = "X-G" ] || [ "X$1" = "X--giturl" ] || [ "X$1" = "X--GITURL" ]; then
		if [ "X${OPT_GIT_URL}" != "X" ]; then
			echo "[ERROR] ${PRGNAME} : already set \"--giturl(-g)\" option."
			exit 1
		fi
		shift
		if [ $# -eq 0 ]; then
			echo "[ERROR] ${PRGNAME} : \"--giturl(-g)\" option is specified without parameter."
			exit 1
		fi
		OPT_GIT_URL=$1
...
if [ "X${OPT_GIT_URL}" != "X" ]; then
	GIT_DOMAIN_URL=${OPT_GIT_URL}
else
	if [ "X${GITHUB_SERVER_URL}" != "X" ]; then
		GIT_DOMAIN_URL=${GITHUB_SERVER_URL}
	else
		GIT_DOMAIN_URL="https://github.com"
	fi
fi
...
if [ ! -f ${PACKAGEJSON_FILEPATH} ]; then
	echo "[ERROR] ${PRGNAME} : ${PACKAGEJSON_FILEPATH} is not existed."
	exit 1
fi
PACKAGEJSON_PACKAGE_NAME=$(grep '"name":' package.json | head -1 | sed -e 's/"//g' -e 's/,//g' -e 's/name://g' -e 's/ //g')
PACKAGEJSON_PACKAGE_VERSION=$(grep '"version":' package.json | head -1 | sed -e 's/"//g' -e 's/,//g' -e 's/version://g' -e 's/ //g')
...
if [ "X${IMAGE_VERSION}" = "X" ]; then
	if [ "X${TAGGED_VERSION}" != "X" ]; then
		...
		# From package.json
		#
		if [ "X${PACKAGEJSON_PACKAGE_VERSION}" != "X" ]; then
			IMAGE_VERSION="${PACKAGEJSON_PACKAGE_VERSION}"
		else
			IMAGE_VERSION="0.0.0"
		fi
	fi
...
echo "  GIT_DOMAIN_URL           = ${GIT_DOMAIN_URL}"
...
if [ "X${PKG_REPO_SETUP_NODEJS}" != "X" ]; then
...
if [ "X${PKG_INSTALL_LIST_NODEJS}" != "X" ]; then
...
cat ${SRCTOP}/templ/${DOCKER_TEMPL_FILE} |									\
	sed -e "s#%%DOCKER_IMAGE_NAME%%#${PACKAGEJSON_PACKAGE_NAME}#g"			\
		-e "s#%%GIT_DOMAIN_URL%%#${GIT_DOMAIN_URL}#g"						\
		-e "s#%%DOCKER_IMAGE_BASE%%#${DOCKER_IMAGE_BASE}#g"					\
